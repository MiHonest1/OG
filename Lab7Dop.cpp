...
//Функция для создания нужного графа в виде матрицы смежности // Weight = макимально возможный вес// Way = 1 / 0(Направлен граф или нет)// Size - размер
void CreateGraphFunc(int Weight, int Way, int Size) {

	p = (int**)calloc(Size, sizeof(int));                             //Выделяем место под динамическую матрицу
	for (i = 0; i < n; i++) { 
		p[i] = (int*)calloc(Size, sizeof(int)); //Выделение места для строк динамической матрицы
	} 
...
int main(int argc, char* argv[]) {
	...
	if (argc <= 3) { 
		cout << "Отсутствуют параметры для работы программы.\nАвтоматическое создание." << endl; 
		CreateGraphFunc(2, 0, 7); 
	}
	else { 
		Wei = atoi(argv[1]); 
		Way = atoi(argv[2]); 
		n = atoi(argv[3]); 
		CreateGraphFunc(Wei, Way, n); 
	}
